...
class UserService:
    ...
    async def update_subscribe(cls, tg_id):
        ...
        async with pool.acquire() as con:
            return await con.execute(
                """UPDATE users SET is_subscribed = False WHERE telegram_id = $1""", tg_id
            )
    ...
    @classmethod
    async def return_args(cls, tg_id):
        pool = cls.get_engine().pool

        async with pool.acquire() as con:
            result = await con.fetch(
                """SELECT name, surname, nickname FROM users WHERE telegram_id = $1""", tg_id
            )
            if result:
                # Распаковываем значения в переменные
                name, surname, nickname = result[0]['name'], result[0]['surname'], result[0]['nickname']
